...
;;
;; `U' (ampc-seek): Seek within the current song.
;;
...
(defvar ampc-views
  (let* ((songs '(1.0 song :properties (("Track" :title "#" :width 4)
         ...
         (rs_b `(1.0 vertical
                     ...
                     ,songs))
         (status '(12 status :cursor-nonselected nil))
    ...
    `((tagger
       ...
       (0.65 files-list
             ...
             :dedicated nil)
       (1.0 tagger))
      (("Current playlist view (Genre|Artist|Album)" . ,(kbd "J"))
       horizontal
       (0.4 vertical
            ,status
            (1.0 current-playlist ,@pl-prop))
       ,rs_a)
      (("Current playlist view (Genre|Album|Artist)" . ,(kbd "M"))
       horizontal
       (0.4 vertical
            ,status
            (1.0 current-playlist ,@pl-prop))
       ,rs_b)
      (("Playlist view (Genre|Artist|Album)" . ,(kbd "K"))
       horizontal
       (0.4 vertical
            ,status
            (1.0 vertical
                 ...
                 (1.0 playlists)))
       ,rs_a)
      (("Playlist view (Genre|Album|Artist)" . ,(kbd "<"))
       horizontal
       (0.4 vertical
            ,status
            (1.0 vertical
...
(defvar ampc-last-status-update nil)
...
(defvar ampc-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map (kbd "U") 'ampc-seek)
    ...
    (cl-loop for (desc . rest) in ampc-views
          do (when (consp desc)
               (define-key map (cdr desc)
                 `(lambda ()
                    (interactive)
                    (ampc-change-view (cdr (assoc ',desc ampc-views)))))))
    map))
...
(defvar ampc-files-list-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map (kbd "M-n") 'ampc-tagger-next-line)
...
(easy-menu-define nil ampc-mode-map nil
  `("ampc"
    ("Change view"
     ,@(cl-loop for (desc . rest) in ampc-views
             when (consp desc)
             collect (vector (car desc)
                             `(lambda ()
                                (interactive)
                                (ampc-change-view
                                 (cdr (assoc ',desc ampc-views)))))
             end))
    ["Run tagger" ampc-tagger]
    "--"
    ["Play" ampc-toggle-play :visible (not (ampc-playing-p))]
    ["Pause" ampc-toggle-play :visible (ampc-playing-p)]
    ["Stop" (lambda () (interactive) (ampc-toggle-play 4))
     :visible (ampc-playing-p)]
    ["Next" ampc-next]
...
;;; *** internal functions
(defun ampc-debug (message &rest args)
  (when ampc-debug
    (apply 'message (concat "ampc (%s): " message)
           (format-time-string "%M:%S") args)))

(defun ampc-tagger-report (args status)
  (unless (zerop status)
    (let ((message (format "ampc_tagger (%s %s) returned with a non-zero exit \
status (%s)"
...
(defun ampc-send-command-impl (command)
  (ampc-debug "-> %s" command)
  (when (ampc-on-p)
    (process-send-string ampc-connection (concat command "\n"))))

(cl-defun ampc-send-command
    (command &optional props &rest args &aux (inhibit-quit t))
  (cl-destructuring-bind (&key (front nil) (keep-prev nil) (full-remove nil)
                            (remove-other nil) (discard-working-timer nil)
                            &allow-other-keys
                            &aux idle)
      props
...
(cl-defun ampc-transform-time (&rest time &aux result)
  (cl-loop for ts in time
        do (when ts
             (push (if (stringp ts)
                       (string-to-number ts)
                     ts)
                   result)))
  (when result
    (cl-callf2 apply '+ result)
    (cl-callf round result)
    (concat (number-to-string (/ result 60))
            ":"
            (when (< (% result 60) 10)
              "0")
            (number-to-string (% result 60)))))
...
(defun ampc-handle-setup (status)
  (unless (and (string-match "^ MPD \\(.+\\)\\.\\(.+\\)\\.\\(.+\\)$"
               ...
               (let ((version-a (string-to-number (match-string 1 status)))
                 ...
                 (or (> version-a 0)
                     (>= version-b 15))))
    (error "Your version of MPD is not supported.  \
ampc supports MPD protocol version 0.15.0 and later")))
...
(defun ampc-handle-status ()
  (setf ampc-last-status-update (float-time))
  ...
  (when ampc-status-timer
    ...
    (setf ampc-status-timer nil))
  (when (ampc-playing-p)
    (setf ampc-status-timer
          (run-at-time
           0.5
           nil
           (lambda ()
             (when (ampc-playing-p)
               (ampc-send-command 'status '(:discard-working-timer t)))))))
  (ampc-fill-status)
  (run-hook-with-args ampc-status-changed-hook ampc-status))

(defun ampc-playing-p ()
...
(cl-defun ampc-configure-frame-1 (split &aux (split-type (car split)))
  (if (memq split-type '(vertical horizontal))
    ...
    (cl-destructuring-bind
        (&key (properties nil) (dedicated t) (mode-line t) (cursor cursor-type)
              (cursor-nonselected cursor-in-non-selected-windows)
              &allow-other-keys)
        (cdr split)
      (apply 'ampc-set-tab-offsets properties)
      (setf ampc-type split
            cursor-type cursor
            cursor-in-non-selected-windows cursor-nonselected
...
;;; *** interactives
(defun ampc-seek (&optional where)
  "Seek to position WHERE within the song that is currently played.
WHERE can be an integer which specifies the absolute position in
seconds or a floating point number within the range 0 and 1 which
specifies the position to seek to proportionately to the song
duration."
  (interactive)
  (unless where
    (setf where (read-number "Seeks to position: ")))
  (when (floatp where)
    (cl-callf * where (string-to-number (cdr (assq 'Time ampc-status)))))
  (ampc-send-command 'seek '(:full-remove t)
                     (cdr (assq 'song ampc-status)) (round where)))
...
zero-indexed position of the current playlist."
  ...
  (unless (or arg (eobp))
    (setf arg (1- (line-number-at-pos))))
  (when arg
    (ampc-send-command 'play '(:remove-other (play)) arg)
...
(cl-defun ampc-status (&optional no-print (width 40))
...
have enough information yet.  WIDTH, which defaults to 40,
specifies a guideline value for the amount of character that
should be used per line."
  ...
  (let* ((flags (mapconcat
         ...
         (stopped (equal (cdr (assq 'state ampc-status)) "stop"))
         (status
          (concat
           "State:     " state
           (when (and ampc-working-timer no-print)
           ...
           (unless stopped
             (concat
              "\n"
              "Playing:   "
              (ampc-clean-tag 'Artist (cdr (assq 'Artist ampc-status)))
              " - "
              (ampc-clean-tag 'Title (cdr (assq 'Title ampc-status)))
              "\n"
              "Elapsed:   "
              (ampc-transform-time (cdr (assq 'elapsed ampc-status))
                                   (when (and (ampc-playing-p)
                                              ampc-last-status-update)
                                     (- (float-time) ampc-last-status-update)))
              (let ((time (ampc-transform-time (cdr (assq 'Time ampc-status)))))
                (when time
                  (concat " (" time ")\n")))
              "\n"))
           ...
           (unless (equal flags "")
             (concat "\n" flags))
           (unless stopped
             (let ((time (cdr (assq 'Time ampc-status)))
                   (elapsed (cdr (assq 'elapsed ampc-status))))
               (when (and time elapsed)
                 (setf elapsed (float (string-to-number elapsed)))
                 (when (and (ampc-playing-p) ampc-last-status-update)
                   (cl-incf elapsed (- (float-time) ampc-last-status-update)))
                 (concat
                  "\n\n["
                  (cl-loop with offset = (/ elapsed
                                         (float (string-to-number time)))
                        with current-offset = 0.0
                        for (char . count) in
                        (list (cons ?= (floor (* offset (- width 2))))
                              (cons ?> 1)
                              (cons ?- (1- (ceiling (* (- 1 offset)
                                                       (- width 2))))))
                        concat
                        (cl-loop
                         repeat count
                                     ...
                                     (let ((map (make-sparse-keymap)))
                                       (define-key map (kbd "<down-mouse-1>")
                                         (let ((current-offset current-offset))
                                           (lambda ()
                                             (interactive)
                                             (ampc-seek current-offset))))
                                       map))
                         concat (propertize " " 'invisible t)
                         do (cl-incf current-offset (/ 1.0 (- width 2)))))
                  "]")))))))
    (unless no-print
      (message "%s" status))
    status))
    ...
    (with-temp-buffer
      ...
      (let ((version (buffer-substring (line-beginning-position)
        ...
        (unless (equal version ampc-tagger-version)
          (message "The reported version of %s is not supported - \
got \"%s\", want \"%s\""
                   ampc-tagger-executable
                   version
                   ampc-tagger-version)
  ...
  (unless ampc-tagger-music-directories
    (message "ampc-tagger-music-directories is nil.  \
Fill it via M-x customize-variable RET ampc-tagger-music-directories RET")
    (cl-return-from ampc-tagger))
...
(defun ampc-tagger-move-beginning-of-line (&optional arg)
  "Move to beginning of tag value or line.
...
(defun ampc-tagger-previous-line (&optional arg)
  "Go to the previous ARG'th entry in the files list buffer.
ARG defaults to 1"
  (interactive "P")
  (ampc-with-buffer 'files-list
    no-se
    (ampc-previous-line arg)))

(defun ampc-tagger-next-line (&optional arg)
  "Go to the next ARG'th entry in the files list buffer.
ARG defaults to 1"
  (interactive "P")
  (ampc-with-buffer 'files-list
    no-se
...
default to the ones specified in `ampc-default-server'."
  ...
  (if suspend
      (ampc-update)
    (ampc-configure-frame (cl-loop for (desc . view) in ampc-views
                                do (when (consp desc)
                                     (cl-return view)))))
